...
pub fn param_list(input: &str) -> Res<&str, ParamList> {
    ...
    context(
        "param_list",
        tuple((param, many0(tuple((char(','), param))))),
    )(input)
    .map(|(next_input, res)| {
...
pub fn param(input: &str) -> Res<&str, Param> {
    ...
    context("param", tuple((char('$'), eq_name, opt(type_declaration))))(input).map(
        |(next_input, res)| {
            (
                next_input,
                Param {
...
pub fn type_declaration(input: &str) -> Res<&str, TypeDeclaration> {
    ...
    context("type_declaration", tuple((tag("as"), sequence_type)))(input)
